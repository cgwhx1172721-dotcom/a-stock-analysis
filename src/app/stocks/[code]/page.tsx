...
function sigCard(s:'good'|'neutral'|'bad') { return s==='good'?'border-orange-200 bg-orange-50':s==='bad'?'border-rose-200 bg-rose-50':'border-[#E5E5EA] bg-white'; }
function sigText(s:'good'|'neutral'|'bad') { return s==='good'?'text-orange-700':s==='bad'?'text-rose-600':'text-[#1A1A1E]'; }
...
function Skeleton() {
  return (
    <div className="min-h-screen bg-[#F2F2F7] p-4 space-y-3 animate-pulse">
      <div className="h-28 rounded-2xl bg-[#E5E5EA]" />
      ...
      {[...Array(4)].map((_,i)=><div key={i} className="h-32 rounded-2xl bg-[#E5E5EA]"/>)}
    </div>
  );
}
  ...
  if(error||!basic) return (
    <main className="min-h-screen bg-[#F2F2F7] flex items-center justify-center px-4">
      <div className="text-center space-y-4">
        ...
        <button onClick={()=>router.push('/')} className="rounded-2xl border border-[#E5E5EA] bg-white px-6 py-3 text-[#1A1A1E] text-sm shadow-sm">← 返回</button>
      </div>
    </main>
  );
  ...
  return (
    <main className="min-h-screen bg-[#F2F2F7] pb-28">
      <div className="max-w-2xl mx-auto px-4 pt-4 space-y-3">
        ...
        {/* Alert banners */}
        {highTriggered&&<div className="rounded-2xl border border-orange-300 bg-orange-50 px-4 py-2.5 text-sm text-orange-700">🎯 目标价 ¥{highNum.toFixed(2)} 已触达！当前 ¥{curPrice.toFixed(2)}</div>}
        {lowTriggered&&<div className="rounded-2xl border border-rose-300 bg-rose-50 px-4 py-2.5 text-sm text-rose-700">⚠️ 预警价 ¥{lowNum.toFixed(2)} 已触达！当前 ¥{curPrice.toFixed(2)}</div>}

        {/* ─── 顶部标题栏 ─── */}
        <section className="rounded-2xl border border-[#E5E5EA] bg-white p-5 shadow-sm">
          <button onClick={()=>router.push('/')} className="text-xs text-[#9A9A9E] hover:text-orange-500 mb-2 transition-colors">← 重新搜索</button>
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right shrink-0">
              ...
              <div className="flex gap-2 mt-2 justify-end">
                <button onClick={load} className="rounded-lg bg-orange-500 hover:bg-orange-400 active:bg-orange-600 px-3 py-1.5 text-xs text-white font-medium shadow-sm transition-colors">刷新</button>
                <button onClick={toggleWatchlist} className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${inWatchlist?'border-orange-300 bg-orange-50 text-orange-600':'border-[#E5E5EA] bg-white text-[#9A9A9E] hover:border-orange-300 hover:text-orange-500'}`}>
                  {inWatchlist?'★ 已自选':'☆ 加自选'}
                </button>
              </div>
            </div>
        ...
        <section className="rounded-2xl border border-[#E5E5EA] bg-white p-4 shadow-sm">
          <div className="grid grid-cols-4 gap-1.5">
            {[
              ...
              <div key={item.label} className="rounded-xl border border-[#F0F0F5] bg-[#F8F8FA] p-2.5 text-center">
                <p className="text-[#9A9A9E] text-xs">{item.label}</p>
        ...
        <div className="grid gap-3 md:grid-cols-[1.4fr_0.6fr]">
          <section className="rounded-2xl border border-[#E5E5EA] bg-white p-4 shadow-sm">
            ...
            <div className="space-y-2">
              {technical&&technical.ma5!=null&&technical.ma20!=null&&(()=>{
                ...
                const info=isUp?{text:'多头排列',desc:'短中期均线向上，趋势健康',color:'text-orange-700',border:'border-orange-200',bg:'bg-orange-50'}:isDn?{text:'空头排列',desc:'三线向下，注意止损',color:'text-rose-600',border:'border-rose-200',bg:'bg-rose-50'}:{text:'均线混乱',desc:'方向不明确，建议观望',color:'text-[#4A4A4E]',border:'border-[#E5E5EA]',bg:'bg-[#F8F8FA]'};
                return(
                  <div className={`rounded-xl border ${info.border} ${info.bg} p-3`}>
                    <div className="flex items-start justify-between"><p className={`text-sm font-bold ${info.color}`}>{info.text}</p><span className="text-xs text-[#9A9A9E]">均线系统</span></div>
                    <p className="text-xs text-[#6A6A6E] mt-0.5">{info.desc}</p>
                    <div className="mt-2 flex flex-wrap gap-3 text-xs text-[#9A9A9E]">
                      <span>MA5 <span className="text-[#1A1A1E] font-medium">{fmtY(m5)}</span></span>
                      {technical.ma20&&<span>MA20 <span className="text-[#1A1A1E] font-medium">{fmtY(technical.ma20)}</span></span>}
                      {technical.ma60&&<span>MA60 <span className="text-[#1A1A1E] font-medium">{fmtY(technical.ma60)}</span></span>}
                    </div>
                  </div>
                );
              })()}
              {technical?.rsi!=null&&(()=>{const r=rsiInfo(technical.rsi!);return(<div className={`rounded-xl border ${r.border} ${r.bg} p-3`}><div className="flex items-center justify-between"><span className="text-xs text-[#9A9A9E]">RSI 强弱指数</span><span className="text-xs text-[#6A6A6E]">{technical.rsi!.toFixed(1)}</span></div><p className={`text-sm font-bold ${r.color} mt-1.5`}>{r.text}</p><p className="text-xs text-[#6A6A6E] mt-0.5">{r.desc}</p></div>)})()}
              {technical?.macd.macd!=null&&(()=>{const m=macdInfo(technical.macd.macd!);return(<div className={`rounded-xl border ${m.border} ${m.bg} p-3`}><div className="flex items-center justify-between"><span className="text-xs text-[#9A9A9E]">MACD 动能</span><span className="text-xs text-[#6A6A6E]">{technical.macd.macd!.toFixed(3)}</span></div><p className={`text-sm font-bold ${m.color} mt-1.5`}>{m.text}</p><p className="text-xs text-[#6A6A6E] mt-0.5">{m.desc}</p></div>)})()}
              {technical?.signals.length?(
                <div className="rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] p-3">
                  ...
                  <ul className="space-y-1.5">{technical.signals.map((s,i)=><li key={i} className="flex items-start gap-2 text-xs text-[#4A4A4E]"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-orange-400 shrink-0"/>{s}</li>)}</ul>
                </div>
              ):null}
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-[#E5E5EA] bg-white p-4 shadow-sm">
            <h3 className="text-sm font-semibold text-[#1A1A1E] mb-3">综合分析</h3>
            <div className="space-y-2">
              {[
                ...
                <div key={i} className={`rounded-xl border ${sigCard(item.sig)} p-2.5`}>
              ...
              {momAdvice&&(
                <div className="rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] p-3 mt-1">
                  <p className="text-xs text-[#9A9A9E] mb-1">综合建议</p>
                  <p className="text-xl">{momAdvice.emoji}</p>
        ...
        {/* ─── 持仓管理卡片 ─── */}
        <section className="rounded-2xl border border-[#E5E5EA] bg-white p-5 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-[#1A1A1E] uppercase tracking-wide">持仓管理</p>
            <button onClick={()=>setShowHoldForm(!showHoldForm)} className="text-xs text-orange-500 hover:text-orange-400 font-medium transition-colors">
              {h?(showHoldForm?'取消':'修改'):(showHoldForm?'取消':'+ 录入持仓')}
            </button>
          </div>

          {showHoldForm&&(
            <div className="space-y-2.5 rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] p-4">
              <input type="number" inputMode="numeric" value={hShares} onChange={e=>setHShares(e.target.value)} placeholder="持股数量（股）"
                className="w-full rounded-xl border border-[#E5E5EA] bg-white px-3 py-2.5 text-sm text-[#1A1A1E] placeholder:text-[#C0C0C5] outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"/>
              <input type="number" inputMode="decimal" value={hCost} onChange={e=>setHCost(e.target.value)} placeholder="持仓均价（元）"
                className="w-full rounded-xl border border-[#E5E5EA] bg-white px-3 py-2.5 text-sm text-[#1A1A1E] placeholder:text-[#C0C0C5] outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"/>
              <div className="flex gap-2">
                <button onClick={saveHoldings} className="flex-1 rounded-xl bg-orange-500 hover:bg-orange-400 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors">保存持仓</button>
                {h&&<button onClick={clearHoldings} className="rounded-xl border border-[#E5E5EA] px-4 py-2.5 text-xs text-[#9A9A9E] hover:text-rose-500 transition-colors">清除</button>}
              </div>
            </div>
          )}

          {!h&&!showHoldForm&&(
            <div className="rounded-xl border border-dashed border-[#E5E5EA] p-5 text-center bg-[#F8F8FA]">
              <p className="text-sm text-[#9A9A9E]">录入持仓数量和成本价</p>
              <p className="text-xs text-[#C0C0C5] mt-1">即可查看止盈止损和盈亏分析</p>
            </div>
          )}
          ...
          {h&&!showHoldForm&&(
            <>
              <div className="grid grid-cols-2 gap-2.5">
                <div className="rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] p-3">
                  <p className="text-xs text-[#9A9A9E] mb-1">当前价</p>
                ...
                <div className="rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] p-3">
                  <p className="text-xs text-[#9A9A9E] mb-1">成本价</p>
                  <p className="text-xl font-bold text-[#1A1A1E]">{fmtY(avgCost)}</p>
                  <p className="text-xs text-[#9A9A9E] mt-0.5">× {shares.toLocaleString()} 股</p>
                </div>
              </div>

              <div className="rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] p-3 flex items-center justify-between">
                <div>
                  <p className="text-xs text-[#9A9A9E] mb-0.5">未实现盈亏</p>
                  <p className={`text-lg font-bold ${pnl>=0?'text-rose-500':'text-emerald-600'}`}>{fmtAmt(pnl)}</p>
                  <p className={`text-xs ${pnl>=0?'text-rose-400':'text-emerald-500'}`}>{pnl>=0?'+':''}{pnlPct.toFixed(2)}%</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-[#9A9A9E] mb-0.5">持仓市值</p>
                  <p className="text-base font-semibold text-[#1A1A1E]">¥{marketValue.toLocaleString('zh-CN',{maximumFractionDigits:0})}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold text-[#1A1A1E] uppercase tracking-wide">止盈止损</p>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${pnlPct>=0?'bg-orange-100 text-orange-600':'bg-rose-100 text-rose-500'}`}>
                    {pnlPct>=0?'+':''}{pnlPct.toFixed(1)}% 浮盈
                  </span>
                </div>

                <div>
                  <p className="text-xs text-rose-500 mb-1.5">🛡 止损位</p>
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between rounded-xl border border-rose-200 bg-rose-50 px-3 py-2">
                      <span className="text-xs text-rose-600 font-semibold">保本止损（成本价）</span>
                      <div className="text-right">
                        <span className="text-sm font-bold text-[#1A1A1E]">{fmtY(hardStop)}</span>
                        <span className="text-xs text-rose-400 ml-1.5">跌破清仓</span>
                      </div>
                    </div>
                    {trailStops.map(ts=>(
                      <div key={ts.label} className={`flex items-center justify-between rounded-xl px-3 py-2 border ${ts.recommended?'border-rose-200 bg-rose-50/70':'border-[#E5E5EA] bg-[#F8F8FA]'}`}>
                        <span className={`text-xs ${ts.recommended?'text-rose-600 font-semibold':'text-[#6A6A6E]'}`}>{ts.recommended?'⭐ ':''}{ts.label}</span>
                        <span className="text-sm font-bold text-[#1A1A1E]">{fmtY(ts.price)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <p className="text-xs text-orange-600 mb-1.5">
                    🎯 止盈目标
                    <span className="text-[#9A9A9E] font-normal ml-1">R = ¥{R.toFixed(2)}/股</span>
                  </p>
                  <div className="grid grid-cols-3 gap-1.5">
                    {rTargets.map(({n,price})=>{
                      const achieved=curPrice>=price;
                      return(
                        <div key={n} className={`rounded-xl p-2 text-center border ${achieved?'border-orange-200 bg-orange-50':'border-[#E5E5EA] bg-[#F8F8FA]'}`}>
                          <p className={`text-xs font-bold ${achieved?'text-orange-600':'text-[#C0C0C5]'}`}>{achieved?'✅':'▷'} {n}R</p>
                          <p className={`text-xs font-semibold mt-0.5 ${achieved?'text-[#1A1A1E]':'text-[#9A9A9E]'}`}>{fmtY(price)}</p>
                        </div>
                      );
                    })}
                  </div>
                ...
                {pnlPct>0&&(lastAchieved||nextTarget)&&(
                  <div className="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2.5 space-y-0.5">
                    <p className="text-xs font-semibold text-sky-600">💡 当前建议</p>
                    {lastAchieved&&lastAchieved.n>=4&&<p className="text-xs text-[#4A4A4E]">已达 {lastAchieved.n}R，考虑减仓 1/3 锁定利润</p>}
                    {nextTarget&&<p className="text-xs text-[#6A6A6E]">下一目标 {nextTarget.n}R {fmtY(nextTarget.price)}，还差 +{((nextTarget.price-curPrice)/curPrice*100).toFixed(1)}%</p>}
                  </div>
                )}
              </div>
            </>
          ...
          {/* 到价提醒 */}
          <div>
            <p className="text-xs font-semibold text-[#1A1A1E] mb-2.5 uppercase tracking-wide">到价提醒</p>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <p className="text-xs text-[#9A9A9E] mb-1">目标价（涨到提醒）</p>
                <input type="number" value={alertHigh} onChange={e=>setAlertHigh(e.target.value)} placeholder="如 15.00"
                  className={`w-full rounded-xl bg-[#F8F8FA] border px-3 py-2 text-sm text-[#1A1A1E] outline-none ${savedHigh?(highTriggered?'border-orange-400':'border-orange-300'):'border-[#E5E5EA]'} focus:border-orange-400 focus:ring-2 focus:ring-orange-100`}/>
                {savedHigh&&<p className={`text-xs mt-1 ${highTriggered?'text-orange-600 font-semibold':'text-[#9A9A9E]'}`}>{highTriggered?'已触达!':'距目标 +'+((highNum-basic.price)/basic.price*100).toFixed(1)+'%'}</p>}
              </div>
              <div>
                <p className="text-xs text-[#9A9A9E] mb-1">预警价（跌到提醒）</p>
                <input type="number" value={alertLow} onChange={e=>setAlertLow(e.target.value)} placeholder="如 9.50"
                  className={`w-full rounded-xl bg-[#F8F8FA] border px-3 py-2 text-sm text-[#1A1A1E] outline-none ${savedLow?(lowTriggered?'border-rose-400':'border-rose-300'):'border-[#E5E5EA]'} focus:border-orange-400 focus:ring-2 focus:ring-orange-100`}/>
                {savedLow&&<p className={`text-xs mt-1 ${lowTriggered?'text-rose-500 font-semibold':'text-[#9A9A9E]'}`}>{lowTriggered?'已触达!':'距预警 -'+((basic.price-lowNum)/basic.price*100).toFixed(1)+'%'}</p>}
              </div>
            </div>
            <button onClick={saveAlert} className="mt-2 w-full rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] hover:bg-[#F0F0F5] py-2 text-xs text-[#4A4A4E] transition-colors">保存提醒</button>
          </div>
          ...
          <div className="rounded-2xl border border-orange-200 bg-orange-50 overflow-hidden">
            <button onClick={()=>{setAiExpanded(!aiExpanded);if(!aiExpanded&&!aiText)loadAi(basic.code,avgCost||undefined,shares||undefined);}}
              ...
              <span className="text-xs font-semibold text-orange-600 uppercase tracking-wide">🤖 DeepSeek AI 分析</span>
              <span className="text-xs text-[#9A9A9E]">{aiExpanded?'收起 ▲':'展开 ▼'}</span>
            </button>
            {aiExpanded&&(
              <div className="px-4 pb-4 border-t border-orange-100">
                {aiLoading&&<p className="text-xs text-orange-400 animate-pulse pt-3">正在生成分析，请稍候…</p>}
                {!aiLoading&&!aiText&&<p className="text-xs text-[#9A9A9E] pt-3">分析加载失败，请重试。</p>}
                {aiText&&(
                  <div className="pt-3">
                    <p className="text-sm text-[#4A4A4E] leading-7 whitespace-pre-line">{aiText}</p>
                    {aiAnalyzedAt&&<p className="text-xs text-[#C0C0C5] mt-2">✨ {aiAnalyzedAt}</p>}
                  </div>
                )}
              </div>
            )}
          ...
          {/* ─── 今日相关公告 ─── */}
          <div>
            <p className="text-xs font-semibold text-[#1A1A1E] mb-2.5 uppercase tracking-wide">今日相关公告</p>
            {newsLoading&&<p className="text-xs text-[#C0C0C5] animate-pulse">加载中...</p>}
            {!newsLoading&&news.length===0&&<p className="text-xs text-[#C0C0C5]">暂无最新公告</p>}
            <div className="space-y-2">
              {news.map((item,i)=>(
                <a key={i} href={item.url} target="_blank" rel="noopener noreferrer"
                  className="block rounded-xl border border-[#E5E5EA] bg-[#F8F8FA] p-3 hover:border-orange-200 hover:bg-orange-50 transition-colors">
                  <p className="text-xs text-[#1A1A1E] leading-relaxed">{item.title}</p>
                  {item.publishTime&&<p className="text-xs text-[#9A9A9E] mt-1">{item.source} · {item.publishTime}</p>}
                </a>
              ))}
            </div>
          </div>
